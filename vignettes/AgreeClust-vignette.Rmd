...
For implementing the cutting strategy of the dendrogram, the Likelhood-Ratio Test is used as the statistic test to test the significance of the $(K+1)-$ latent class model w.r.t the $K-$ latent class model
...
As shown on the help page, the function uses by default the following important arguments:
...
By default, the `AgreeClustBin` function gives two graphs, one illustrating the clustering process and one illustrating the multidimensional structure of disagreement observed among the panel.
