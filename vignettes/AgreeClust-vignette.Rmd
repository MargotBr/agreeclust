...
<center>$d^{\scriptscriptstyle \text{res}}_{rr'} = \sqrt{\sum_{s=1}^{S}(\hat{\varepsilon}_{sr} - \hat{\varepsilon}_{sr'} )^{2}}$</center>
...
### Interpretation of the clusters by using external information about the raters
...
#### Description of the clusters by the values of a quantitative variable describing the raters
...
#### Description of the clusters by the levels of a categorical variable describing the raters
...
### Interpretation of the clusters by using external information about the stimuli
...
#### Description of the clusters by the values of a quantitative variable describing the stimuli
...
<center>$g(Y_{srk})=\beta_{0}+\beta_{1}x+\delta_{k}+\beta_{2k}x$ with $\sum_{k=1}^{K}\delta_{k}=0$ and $\sum_{k=1}^{K}\beta_{2k}=0$ </center>

where $g$ is the link function; $Y_{srk}$ stands for the rating of the $s$-th stimulus, with $s=1,\ldots,S$, by the $r$-th rater in cluster ${\cal C}_{k}$; $\beta_{1}$ stands for the coefficient associated to the effect of the quantitative variable $X$; $\delta_{k}$ stands for the coefficient associated to the effect of cluster ${\cal C}_{k}$; and $\beta_{2k}$ stands for the interaction effect coefficient between the quantitative variable $X$ and cluster ${\cal C}_{k}$. 

If the global effect of the interaction between the cluster and the quantitative variable $X$ is significant, that means that the effect of the quantitative variable $X$ on the rating is not the same from one cluster to the other. To highlight these aspects, the significance of the coefficients $\beta_{2k}$ is studied. Indeed, if the coefficient $\beta_{2k}$ is significant, that means that the impact of the values of the quantitative $X$ on the rating is significantly different for cluster ${\cal C}_{k}$ w.r.t. the observations made on the entire panel. If this is the case, that means that the impact of the quantitative variable $X$ on the rating characterizes the cluster ${\cal C}_{k}$. The sign of the coefficient is then used to determine if large values of the quantitative variable $X$ led to high ratings (positive coefficient) or low ratings (negative coefficient) in cluster ${\cal C}_{k}$ w.r.t. the observations made on the entire panel.

#### Description of the clusters by the values of a categorical variable describing the stimuli
...
The `AgreeClustCont` function considers an analysis of variance-based latent class regression modeling for the agreement-based unsupervised clustering of a set of continuous ratings.
