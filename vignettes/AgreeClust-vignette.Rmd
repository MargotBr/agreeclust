...
The first object returned by the function is the matrix containing the profiles of deviance residuals. The larger the residual (in absolute value), the larger the impact on the gap from the situation of *perfect agreement* (cf. model (2)). This object can be accessed with this code:

```{r}
res.pedag$profiles.residuals
```

The second object returned by the function is the disagreement matrix between raters. This object can be accessed with this code:

```{r}
res.pedag$mat.disag
```

The first graph created by the function shows the clustering process applied to this disagreement matrix. The dendrogram representing the structure of disagreement is presented at the top of the graph. The $p-$values associated to the different levels of its hierarchy show that two clusters of raters exist among the panel, as the $3-$ latent class model is not significant w.r.t the $2-$ latent class model. The consolidation step, whose the result is presented at the bottom of the graph, did not modify the partition of clusters. In other words, this panel is composed of two disagreed clusters: the first ten raters (cluster 1) against the last ten raters (cluster 2).
...
The results of the clustering process ($p-$values associated to the test of the significance of each $K-$ clusters structure, number of clusters of raters found, and the final partition of raters that is consolidated in the present example) can be accessed with this code:
...
This graph allows to detect the number of dimensions interesting for the interpretation of the structure of disagreement. The user can then plot the graph for the other interesting dimensions, let's say the third and the fourth dimensions:
